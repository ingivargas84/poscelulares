var tiendas_table = $('#tiendas-table').DataTable({
    "responsive": true,
    "processing": true,
    "info": true,
    "showNEntries": true,
...
function confirmarAccion(button) {
	...
	$.ajax({
		...
		url: APP_URL+"/tiendas/" + id + "/delete",
		data: formData,
		dataType: "json",
		success: function(data) {
            $('.loader').fadeOut(225);
			$('#modalConfirmarAccion').modal("hide");
			tiendas_table.ajax.reload();
			alertify.set('notifier','position', 'top-center');
			alertify.success('La Tienda se Desactivó Correctamente!!');
		},
...
$(document).on('click', 'a.activar-tienda', function(e) {
    e.preventDefault(); // does not go through with the link.

    var $this = $(this);
    alertify.confirm('Activar Tienda', 'Esta seguro de activar la tienda',
        function(){
            $('.loader').fadeIn();
            ...
            $.post({
                ...
                tiendas_table.ajax.reload();
                    alertify.set('notifier','position', 'top-center');
                    alertify.success('Tienda Activado con Éxito!!');
            });
         }
        , function(){
            alertify.set('notifier','position', 'top-center');
